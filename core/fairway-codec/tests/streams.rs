...
fn jsonl_stream_traits_round_trip_every_boundary() {
    ...
    for value in &values {
        StreamEncode::encode(&mut encoder, value.clone(), &mut bytes).unwrap();
    }
    ...
    assert!(StreamEncode::encode(&mut encoder, values[0].clone(), &mut bytes).is_err());
    assert_eq!(bytes, before_finish);
    ...
    <T as Encode>::Error: std::fmt::Debug,
{
    for split in 0..=input.len() {
        ...
        decoder.push(input[..split].to_vec()).unwrap();
        ...
        assert!(decoder.next().unwrap().is_none());
        let expected = T::decode(input.to_vec()).unwrap().encode().unwrap();
        ...
        assert_eq!(&output[6..], expected.as_slice());
    }
}
...
fn document_adapters_cover_json_toml_markdown_text_and_bytes() {
    ...
    let mut text = Encoder::<String>::new();
    let mut bytes = Vec::new();
    text.encode("текст".to_owned(), &mut bytes).unwrap();
    text.finish(&mut bytes).unwrap();
    assert_eq!(bytes, "текст".as_bytes());

    let mut raw = Encoder::<Vec<u8>>::new();
    let mut bytes = Vec::new();
    raw.encode(vec![0xff, 0], &mut bytes).unwrap();
...
fn document_decoding_waits_for_eof_and_does_not_accept_truncated_values() {
    ...
    assert!(json.next().unwrap().is_none());
    assert!(json.push(b"2]".to_vec()).is_err());
...
fn custom_types_need_neither_serde_nor_send_and_can_use_custom_errors() {
    ...
    impl Decode for Local {
        type Error = u8;
        fn decode(bytes: Vec<u8>) -> Result<Self, u8> {
            Ok(Self(std::rc::Rc::new(
                String::from_utf8(bytes).map_err(|_| 1_u8)?,
            )))
        }
    }
    impl Encode for Local {
        ...
        fn encode(self) -> Result<Vec<u8>, Infallible> {
            Ok(std::rc::Rc::unwrap_or_clone(self.0).into_bytes())
        }
    }
