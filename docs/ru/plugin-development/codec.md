...
`Markdown` содержит разобранный документ. Метод `events` возвращает его элементы:
текст, начало и конец заголовка, абзаца, списка и других конструкций.
`encode` преобразует документ обратно в Markdown.
...
Кодирование сохраняет структуру документа, но может изменить отступы, маркеры
и экранирование. Для сохранения исходного текста используйте `String`.
...
- `parse(text: &str) -> Markdown` — разбирает любой корректный текст UTF-8.
- `events() -> &[markdown::Event<'static>]` — возвращает элементы документа.
...
- `codec::decode` и `codec::encode` передают преобразование в отдельный вычислительный пул.
...
Ожидание в очереди можно отменить. Начатое преобразование продолжает работу,
удерживая данные и место в пуле до завершения. Если ожидание отменено, результат отбрасывается.
